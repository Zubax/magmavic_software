...
void platform_adc_setup(void)
{
    ...
    DIDR0  = 0xFF;  // Disable digital input buffers on A0-A7 (ADC0-ADC7)

    // Set A0-A5 as inputs (A6-A7 are analog-only, no DDR)
    DDRC &= ~(0x3F);  // Clear PC0-PC5 (A0-A5)
}
...
{
    // Set D10 (PB2) and D11 (PB3) as outputs
    DDRB |= (1 << PB2) | (1 << PB3);

    // Configure Timer1 for D10 (OC1B): Fast PWM, non-inverted, TOP=0xFF (8-bit), prescaler=256
    TCCR1A = (1 << COM1B1) | (1 << WGM10);  // COM1B1:0=10 (non-inverted PWM on OC1B), WGM=5 (Fast PWM 8-bit)
    TCCR1B = (1 << WGM12) | (1 << CS12);  // CS=256
    OCR1B = 0; // 0% duty cycle

    // Configure Timer2 for D11 (OC2A): Fast PWM, non-inverted, prescaler=256
    TCCR2A = (1 << COM2A1) | (1 << WGM21) | (1 << WGM20);  // COM2A1:0=10 (non-inverted), WGM=3 (Fast PWM)
    TCCR2B = (1 << CS22) | (1 << CS21);  // CS=256
...
void platform_pwm_set(const uint8_t channel, const uint8_t duty_pct)
{
    if (channel == 0)
    {
        OCR1B = 255 * (uint32_t) duty_pct / 100; // 255 is 100% duty cycle
    }
    else if (channel == 1)
    {
        OCR2A = 255 * (uint32_t) duty_pct / 100;
    }
}
